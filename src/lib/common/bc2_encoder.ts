...
export namespace BC2Encoder {
    const rWeight = 0.3;
    const gWeight = 0.59;
    const bWeight = 0.11;

    export class BC2Block extends BC1Encoder.BC1Block {
        constructor(
            private alphas = 0n,
            color0: Color.Pixel565, 
            color1: Color.Pixel565
        ) {
            super(color0, color1);
        }

        public get alpha(): bigint {
            return this.alphas;
        }
         ...
         */
        public setAlpha(index: number, alpha: number) {
			const shift = BigInt((4 * (15 - index)));
			const a = BigInt(Math.floor(alpha / 17)) & 0b1111n;
        ...
        public getAlpha(index: number): number {
            const shift = BigInt((4 * (15 - index)));
            return Number((this.alphas >> shift) & 0b1111n) * 17;
		}
    }
     ...
     */
    export function encode(imageData: DataView, width: number, height: number, format: 'DXT2' | 'DXT3'): DDSEncoder.DDSData {
        ...
        return DDSEncoder.DDSData.fromData(output, width, height, format);
    }

    export function decode(imageData: DataView, width: number, height: number, format: 'DXT2' | 'DXT3'): Uint8Array {
        ...
        for (let h = 0; h < height_4; h++) {
            for (let w = 0; w < width_4; w++) {
                ...
                const color0 = Color.Pixel.fromRgb565(imageData.getUint16(offset + 8, true));
                const color1 = Color.Pixel.fromRgb565(imageData.getUint16(offset + 10, true));
    ...
    function encodeBlock(block: Image.RawBlock, format: 'DXT2' | 'DXT3'): BC2Block {
        const [min, max] = block.getMinMaxColorsBC1();

        const color0 = new Color.Pixel(max.red, max.green, max.blue, max.alpha);
        const color1 = new Color.Pixel(min.red, min.green, min.blue, min.alpha);
