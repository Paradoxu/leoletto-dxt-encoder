...
export namespace Image {
    export class RawBlock {
        public readonly pixels = new Array<Color.Pixel>(16);

        constructor(fillColor: Color.Pixel) {
            this.pixels.fill(fillColor);
        }

        public get hasTransparentPixels(): boolean {
            return this.pixels.some(c => c.alpha < 255);
        }

        public setColor(index: number, color: Color.Pixel) {
            this.pixels[index] = color;
        }

        public getMinMaxColorsBC1(): [Color.Pixel, Color.Pixel] {
            ...
            return [
                new Color.Pixel(minR, minG, minB, minA),
                new Color.Pixel(maxR, maxG, maxB, maxA),
            ];
        }
    }

    export function imageTo4x4(image: DataView, width: number, height: number) {
        const blockWidth = ((width + 3) & ~3) >> 2;
        const blockHeight = ((height + 3) & ~3) >> 2;
        const blocks = new Array<RawBlock>(blockWidth * blockHeight);
        let blockOffset = 0;

        for (let h = 0; h < blockHeight; h++) {
            for (let w = 0; w < blockWidth; w++) {
                const block = new RawBlock(new Color.Pixel(0, 0, 0, 0));
                ...
                for (let y = 0; y < 4; y++) {
                    for (let x = 0; x < 4; x++) {
                        ...
                        block.setColor(pixelIndex, new Color.Pixel(red, green, blue, alpha));
                    }
                }
